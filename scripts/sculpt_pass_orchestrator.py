...
from sculpt_contract import (
    DEFAULT_PASS_ORDER,  # compatibility re-export
    SCULPT_DNA_PRODUCTION_PASS_ORDER,
    VISUAL_PASS_IDS,  # compatibility re-export
    ...
    pipeline_status,
    review_failures as contract_review_failures,
    sync_pipeline as contract_sync_pipeline,
    write_spec_atomic,
)
...
LEGACY_VISUAL_PASS_IDS = frozenset(SCULPT_DNA_PRODUCTION_PASS_ORDER[:-1])


def uses_sculpt_dna_v2_contract(spec: dict[str, Any]) -> bool:
    policy = spec.get("reviewPolicy")
    return spec.get("schemaVersion") == "2.0" or (
        isinstance(policy, dict) and policy.get("version") == 2
    )
...
def review_completes_pass(
    ...
    """Evaluate either the published v2 policy or the adaptive v3/v4 contract."""

    if uses_sculpt_dna_v2_contract(spec):
        from visual_evidence_hashes import review_visual_evidence_failures
        from visual_feature_gate import feature_gate_failures

        if entry.get("passId") != pass_id or entry.get("action") != "continue":
            return False
        if pass_id in LEGACY_VISUAL_PASS_IDS:
            ...
            if (
                not has_number(score)
                or not has_number(threshold)
    ...
    return not contract_review_failures(spec, entry, pass_id, spec_path)
...
def completed_passes(
    ...
    for pass_id in ids:
        if uses_sculpt_dna_v2_contract(spec):
            from visual_evidence_hashes import latest_review_for_pass
            ...
            entry = next(
                (
                    ...
                    if isinstance(item, dict) and item.get("passId") == pass_id
                ),
                None,
...
def sync_pipeline(
    ...
    """Synchronize the correct derived pipeline shape for each spec contract."""

    if not uses_sculpt_dna_v2_contract(spec):
        return contract_sync_pipeline(spec)
    ids = pass_order(spec)
    ...
    pipeline.update(
        {
            ...
            "blockedReason": (
                "all build passes completed" if current == "complete" else ""
            ),
            "nextRequiredEvidence": (
                [] if current == "complete" else next_required_evidence(spec, current)
            ),
        }
    )
    return pipeline
...
def pass_specific_evidence(
    spec_or_pass_id: dict[str, Any] | str,
    pass_id: str | None = None,
) -> list[str]:
    if pass_id is not None:
        if isinstance(spec_or_pass_id, dict) and uses_sculpt_dna_v2_contract(
            spec_or_pass_id
        ):
            from sculpt_pass_orchestrator_legacy import (
                pass_specific_evidence as legacy_evidence,
            )

            return legacy_evidence(spec_or_pass_id, pass_id)
        selected_pass = pass_id
    else:
        selected_pass = str(spec_or_pass_id)
    if selected_pass in {"structure", "form", "structural-pass", "form-refinement"}:
        return ["child joints have explicit attachment contracts and no visible floating roots"]
    if selected_pass in {"lookdev", "material-pass", "surface-pass", "lighting-pass"}:
        return ["palette, material response, local detail, lighting, and contact shadow are reviewable"]
    if selected_pass in {"interaction", "interaction-pass"}:
        return ["runtime checks cover load, transforms, and the requested interaction"]
    if selected_pass in {"optimization", "optimization-pass"}:
...
def pass_specific_gaps(spec: dict[str, Any], pass_id: str) -> list[str]:
    if uses_sculpt_dna_v2_contract(spec):
        from sculpt_pass_orchestrator_legacy import pass_specific_gaps as legacy_gaps

        return legacy_gaps(spec, pass_id)
