...
def _sphere_expression(parameters: Mapping[str, Any]) -> str:
    width_segments = _integer(parameters.get("widthSegments"), 64, minimum=3)
    height_segments = _integer(parameters.get("heightSegments"), 40, minimum=2)
    if not any(
        key in parameters
        for key in ("phiStart", "phiLength", "thetaStart", "thetaLength")
    ):
        return (
            "new THREE.SphereGeometry(0.5, "
            f"{width_segments}, {height_segments})"
        )
    return (
        "new THREE.SphereGeometry(0.5,"
        f"{width_segments},"
