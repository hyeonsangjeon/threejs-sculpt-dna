...
def load_spec(path: Path) -> dict[str, Any]:
    try:
        ...
        match = re.search(r"contains non-finite number (\S+)", str(exc))
        if match:
            raise ValueError(
                f"non-standard JSON constant {match.group(1)}"
            ) from exc
        raise
...
def validate_spec(
    spec: dict[str, Any],
    for_pass: str | Path | None = None,
) -> tuple[list[str], list[str]]:
    if spec.get("schemaVersion") == "2.0":
        # Sculpt DNA shipped a stricter evidence-bound schema-2 profile before
        # the adaptive v3/v4 kernel existed. Keep that published contract
        # intact instead of silently reinterpreting its richer legacy fields.
        from validate_sculpt_spec_legacy import validate_spec as validate_legacy_spec

        spec_path = for_pass if isinstance(for_pass, Path) else None
        return validate_legacy_spec(spec, spec_path)

    # Policy-v2 callers historically passed the spec path as the second
    # argument so local evidence could be resolved by companion gates. Keep
    # that API readable while the adaptive v3/v4 validator uses pass IDs.
    if isinstance(for_pass, Path):
        for_pass = None
    errors: list[str] = []
    warnings: list[str] = []
