...
from append_sculpt_review_legacy import pass_specific_evidence
from sculpt_pass_orchestrator import pass_specific_gaps, sync_pipeline
...
def clamp_score(value: float) -> float:
    """Compatibility alias that now rejects, rather than hides, out-of-range input."""
...
def main(argv: list[str]) -> int:
    if argv:
        ...
        try:
            raw_spec = parse_json(candidate.read_text(encoding="utf-8"), "spec")
        except (OSError, ValueError):
            raw_spec = None
        if isinstance(raw_spec, dict) and raw_spec.get("schemaVersion") == "2.0":
            from append_sculpt_review_legacy import main as legacy_main
