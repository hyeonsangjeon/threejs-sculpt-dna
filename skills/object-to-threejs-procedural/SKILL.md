...
Ask for or infer a readable reference, intended use, and target project before
implementation. The preferred v4 route is `python3 ../../scripts/sculpt.py`;
the monolithic compatibility route remains available for existing Sculpt DNA
projects.

For a complete implementation, leave these reproducible outputs in the target
project:

- `assessment.json` with suitability, complexity, uncertainty, and the quality contract;
- `object-sculpt-spec.json` or a v4 modular manifest with geometry, materials, hierarchy, actions, evidence, and pass state;
- a reusable plain Three.js factory plus runtime node, mesh, socket, collider, and destruction maps;
- browser renders, comparison manifests, and hash-bound review records;
- optional Sculpt DNA variants, a curated family manifest, a regression report, and a host integration report.

Minimum successful prompt:

```text
Reconstruct this reference as an action-ready procedural Three.js model.
Validate the image, preserve generated geometry and runtime metadata, and
compare real browser renders against the source. Do not use an imported mesh.
```

The first executable compatibility steps for a local image are:
...
python3 ../../scripts/new_sculpt_spec.py "Object Name" \
  --image <image> --assessment assessment.json \
  --intended-use browser-prop --quality-profile balanced \
  --layout monolithic --out object-sculpt-spec.json
```

These commands create scaffolds, not production approval. Complete the observed
fields and object-specific quality contract before strict validation or
generation.
...
target, capture standalone and host runtime snapshots from the same named
cameras and verify the immutable render integration contract:
...
python3 ../../scripts/render_integration_contract.py \
  ...
  host-snapshot.json \
  --out integration-report.json --summary
```

Exit `1` blocks production acceptance; exit `2` means the telemetry itself is
invalid. Runtime diagnostics may veto an integration but cannot approve visual
quality. Recapture after fixing the host instead of suppressing a check.
