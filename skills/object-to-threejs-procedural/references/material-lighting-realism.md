...
- reference-derived PBR extraction: for `reference-fidelity`, select one material region with `--crop-pixels X Y WIDTH HEIGHT` or `--crop-normalized X Y WIDTH HEIGHT`, then run `python3 ../../scripts/sculpt.py pbr` with `--material-crop-confirmed` and an explicit `--url-prefix`. The report preserves source dimensions, source SHA-256, both coordinate forms, and a deterministic crop identity. Never patch from a full UI/demo screenshot. Conflicting, tiny, out-of-bounds, background-heavy, mixed-material, or below-target evidence must stop patching.
